...
data.x_dict = {k: torch.cat([v, embedding], dim=-1) for k, v in data.x_dict.items()}
...
model = RGNN(256, args.hidden_channels, dataset.num_classes, args.num_layers,
...
def train_attack(epoch, model, optimizer):
    ...
    for batch_size, n_id, adjs in train_loader:
        ...
        perturb_shape = ((node_type[n_id] == 3).sum(), 256)  ##
        y = y_global[n_id][:batch_size].squeeze()
