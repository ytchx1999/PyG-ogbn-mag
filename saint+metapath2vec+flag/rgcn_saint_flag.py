...
dataset = PygNodePropPredDataset(name='ogbn-mag', root='./mag/')
data = dataset[0]
...
class RGCN(torch.nn.Module):
    ...
    def group_input(self, x_dict, node_type, local_node_idx, perturb=None):
        ...
        for key, x in x_dict.items():
            mask = node_type == key
            h[mask] = x[local_node_idx[mask]] if perturb is None else x[local_node_idx[mask]] + perturb  ## 出错
    ...
    def forward(self, x_dict, edge_index, edge_type, node_type,
        ...
        x = self.group_input(x_dict, node_type, local_node_idx, perturb)
...
# embedding_dim = 128
# x_dim = 128
# onehot_dim = dataset.num_classes
# in_channels = int(embedding_dim + x_dim + onehot_dim)
...
embedding = torch.load('mag_embedding.pt', map_location='cpu')
...
#
# onehot = torch.zeros([x_dict[key2int['paper']].shape[0], dataset.num_classes]).to(device)
# onehot[train_idx, labels[train_idx, 0]] = 1
# onehot = onehot.to(device)
#
# x_dict = {k: torch.cat([v, onehot], dim=-1) for k, v in x_dict.items()}
#
# x_dict = {k: v.to(device) for k, v in x_dict.items()}
...
def train_flag(epoch, model, optimizer):
    ...
    for data in train_loader:
        data = data.to(device)
        # optimizer.zero_grad()
        forward = lambda perturb: model(x_dict, data.edge_index, data.edge_attr, data.node_type, data.local_node_idx,
                                        perturb)[data.train_mask]
        model_forward = (model, forward)
        # out = out[data.train_mask]
        perturb_shape = ((data.node_type == 3).sum(), 256)
